...
public class MyMain {
	...
	public static void main(String[] args) {
		...
		try {
			aFrame.setIconImage(ImageIO.read(new File("src/main/resources/Logo_w_space.png")));
		...
		generate.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				gen.generateMap(localmap, (Integer) groundArea.getValue(),biome);
				...
				localmap.cleanUp();
				if(Lake.isSelected()) {
					int ground = (localmap.getLandNo()+localmap.getMountainNo());
					for (int i = 0; i <  Math.random()*(ground/50); i++) {
						localmap.createLake();
					}
				}
				 if (River.isSelected()){
				...
				System.out.printf("Height : %d Width : %d Ground : %d\n",HeightArea.getValue(),widthArea.getValue(),groundArea.getValue() );
				cnv.repaint();
